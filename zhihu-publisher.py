...
GITHUB_REPO_PREFIX = "https://github.com/SoIncredible/Markdown4Zhihu/tree/master/Data"
COMPRESS_THRESHOLD = 5e5 # The threshold of compression

# The main function for this program
def process_for_zhihu():
    # 尝试utf-8读取，不行再用chardet
    lines = None
    if args.encoding is None:
        try:
            with open(str(args.input), 'r', encoding='utf-8') as f:
                lines = f.read()
            args.encoding = 'utf-8'
            print({'encoding': 'utf-8', 'confidence': 1.0})
    ...
    with open(op.join(args.current_script_data_path, args.input.stem + "_for_zhihu.md"),
              "w+", encoding=args.encoding) as fw:
        fw.write(lines)
...
def formula_ops(_lines):
    ...
    _lines = re.sub(r'(\$)(?!\$)(.*?)(\$)',
                    r' <img src="https://www.zhihu.com/equation?tex=\\2" alt="\\2" class="ee_img tr_noresize" eeimg="1"> ', _lines)
    return _lines
